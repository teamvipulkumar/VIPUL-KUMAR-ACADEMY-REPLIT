...
function useCountdown() {
  const [left, setLeft] = useState(() => {
    ...
    return s ? parseInt(s, 10) : 3600; // 1 hour default
  });
  useEffect(() => {
    if (left <= 0) return;
    const t = setInterval(() => {
...
function Countdown() {
  ...
  const box = (v: number, label: string) => (
    <div className="flex flex-col items-center gap-1">
      <div className="w-16 sm:w-20 h-14 sm:h-16 flex items-center justify-center rounded-lg text-2xl sm:text-3xl font-black tabular-nums text-white"
...
function ImgPlaceholder({ label = "Image", height = 260, className = "" }: { label?: string; height?: number; className?: string }) {
  return (
    <div
      ...
      style={{ minHeight: height, background: "#1e293b", border: "2px dashed #334155" }}
    >
      <ImageIcon className="w-8 h-8" style={{ color: "#475569" }} />
      <span className="text-sm font-medium" style={{ color: "#475569" }}>{label}</span>
      <span className="text-xs" style={{ color: "#334155" }}>Upload Later</span>
    </div>
  );
}
...
function TrustRow() {
  return (
    <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-8 text-sm font-semibold" style={{ color: "#94a3b8" }}>
      <span className="flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4 text-green-400" />Instant Access After Payment</span>
      <span className="flex items-center gap-1.5"><ShieldCheck className="w-4 h-4 text-green-400" />30-Day Money Back Guarantee If You Don't See Value</span>
    </div>
  );
}
...
function Module({ num, title, desc }: { num: number; title: string; desc: string }) {
  return (
    <div className="flex gap-4 p-5 rounded-2xl" style={{ background: "#0f172a", border: "1px solid #1e293b" }}>
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: "1px solid #1e293b" }}>
      <button
        ...
        style={{ background: open ? "#0f172a" : "#080d1a" }}
        onClick={() => setOpen(p => !p)}
      >
      ...
      {open && (
        <div className="px-5 pb-5 text-sm leading-relaxed" style={{ background: "#0f172a", color: "#94a3b8", borderTop: "1px solid #1e293b" }}>
...
export default function OrderPage() {
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#080d1a", color: "#e2e8f0" }}>
      ...
      {/* ── Support Bar ── */}
      <div className="text-center py-2.5 px-4 text-sm font-medium" style={{ background: "#0f172a", borderBottom: "1px solid #1e293b" }}>
        Need Help?{" "}
        <a
      ...
      <main className="flex-1">
          ...
          <div className="max-w-4xl mx-auto px-4 pb-10 space-y-5 mt-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-2xl sm:text-3xl font-black text-white">
                ...
                <span className="font-black" style={{ color: "#3b82f6" }}>JUST ₹299</span>
              </p>
            </div>
        ...
        <Section style={{ background: "#0a1628", borderTop: "1px solid #1e293b", borderBottom: "1px solid #1e293b" }}>
          <h2 className="text-2xl sm:text-3xl font-black text-center text-white mb-10">
            Are You Tired Of...
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={title} className="rounded-2xl p-6 text-center space-y-3" style={{ background: "#0f172a", border: "1px solid #1e293b" }}>
                <div className="w-12 h-12 mx-auto rounded-full flex items-center justify-center text-2xl" style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)" }}>
                  😤
                </div>
        ...
        <Section className="text-center">
          <div className="max-w-3xl mx-auto space-y-4">
            ...
            <p className="text-xl sm:text-2xl font-black leading-tight" style={{ color: "#f59e0b" }}>
        ...
        <Section style={{ background: "#0a1628", borderTop: "1px solid #1e293b", borderBottom: "1px solid #1e293b" }}>
          <div className="max-w-2xl mx-auto text-center space-y-5">
            <div className="w-20 h-20 mx-auto rounded-full flex items-center justify-center" style={{ background: "rgba(34,197,94,0.12)", border: "2px solid rgba(34,197,94,0.3)" }}>
        ...
        <Section>
          <div className="text-center mb-8">
            ...
            <p className="mt-2 text-base font-bold" style={{ color: "#f59e0b" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
            {[
              ...
              <div key={title} className="flex items-center gap-4 p-5 rounded-2xl" style={{ background: "#0f172a", border: "1px solid #1e293b" }}>
                <span className="text-3xl shrink-0">{emoji}</span>
                <div>
                  <p className="font-bold text-white text-sm sm:text-base">{title}</p>
                  <p className="text-xs font-bold mt-0.5" style={{ color: "#22c55e" }}>{value}</p>
                </div>
                <CheckCircle2 className="w-5 h-5 ml-auto shrink-0 text-green-400" />
              </div>
            ))}
          </div>
        </Section>
        ...
        <Section className="text-center">
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="space-y-1">
              ...
              <p className="text-2xl sm:text-3xl font-black text-white">
                For Limited Time Only{" "}
                <span style={{ color: "#3b82f6" }}>JUST ₹299</span>
              </p>
            </div>
